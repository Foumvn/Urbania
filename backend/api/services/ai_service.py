...
class AIService:
    ...
    def generate_notice_descriptive(cls, project_data):
        ...
        - Projet : {project_data.get('descriptionProjet', 'Construction')}
        - Nature : {project_data.get('natureTravaux', [])}
        - Matériaux Façade : {project_data.get('materiauFacade', 'Enduit')} ({project_data.get('couleurFacade', 'Ton pierre')})
        - Matériaux Toiture : {project_data.get('materiauToiture', 'Tuiles')} ({project_data.get('couleurToiture', 'Rouge')})
        - Hauteur : {project_data.get('hauteurConstruction', 'Inconnue')} m
        - Commune : {project_data.get('terrainVille', 'Inconnue')}
        
        La notice doit comporter 3 parties distinctes :
